...
function createRod() {
 ...
 //rod.debug = true;
  ...
  function vilan(){
    //if(frameCount % 1 === 0){
     var ene = createSprite(100,100,10,10);
       ene.addImage(enemeyImg);
      ene.scale = 0.8;
      ene.lifetime = 100;
      
    //  gr.setCollider("circle",0,2,70);
  //   text("Fight with your enemey",500,250)
  //   ene.addImage(enemeyImg);
  //  if(frameCount%1===0){
  //  enemeyCreatedFrame=frameCount;
  //  ene = createSprite =Math.round(random(10,1000));
     
  // if(enemeyCreatedFrame + 10000 ===frameCount && ene){
  //   ene.destroy();
  // }
       enemeyGroup.add(ene);
       ene.x = cannon.x;
